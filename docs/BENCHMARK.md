...
**On the scoring artifacts (#4, #5):** the discipline of pre-registration is that we don't edit the corpus after running. We could have, after the fact, added `alternative` fields to those two tasks and re-run; we chose not to. The honest published number is **94.5%**. The score if half-credit had been triggered on those two tasks as the methodology specifies would be **95.0%** (190.0 / 200). The difference is documented here, not buried.
