...
**Values frozen at record time.** Recording at the *semantic* level — typed tool calls with named arguments — is the right level, but it is necessary, not sufficient. A handler that writes a target-owned value (an enum label, a category ID, a locale-dependent option, a tenant-specific template name) captured when the macro was authored will silently fail against a stricter, multi-tenant, or differently-localized instance of the same surface, where that exact value is not on offer. The argument *values* a handler submits must be resolved at run time from the target's own candidate set, not frozen from the recording. This and the rest of the failure modes that only appear against real surfaces are catalogued in [`AUTHORING_PITFALLS.md`](AUTHORING_PITFALLS.md).
...
- **Macrokit is not RPA.** Robotic Process Automation records UI events at the pixel/keystroke level. The recording is brittle to layout changes and tells the human nothing about what the workflow *means*. Macros are recorded at the semantic level — tool calls with typed arguments — and the recording is the developer-readable encoding of the workflow's intent. (Semantic recording captures the workflow's *structure*; the argument *values* it writes into a target must still be resolved at run time, not frozen — see §8 and [`AUTHORING_PITFALLS.md`](AUTHORING_PITFALLS.md).)
...
Macrokit is live and open (Apache 2.0). The six-package SDK (runtime, LLM adapter layer, browser, authoring kit, reference-data, CLI) is in the repository alongside two reference implementations — a multi-surface GitHub maintainer agent driving five macros against the GitHub API plus one browser-driven macro against the GitHub Actions UI, and a paper-triage agent. The expanded version of this document — adding evaluation against a pre-registered 100-task benchmark and a related-work section — is deposited as a Zenodo preprint with a citable DOI.
