...
The cleanest way to see why this works is that it recapitulates how brains manage the cost of thinking. Deliberation is expensive, so an efficient mind compiles repeated reasoning into cheap automatic reflexes and reserves expensive thinking for novelty — Kahneman's System 2 and System 1. Macrokit ports that split directly: the strong model is System 2 (slow, expensive, for the novel), a macro is System 1 (fast, cheap, deterministic, for the routine), and the whole point is having both with the cheap reflex carrying ~95% of the load. A macro isn't innate instinct, though — it's *compiled deliberation*: a behavior the strong model reasoned out once, chunked down into something a weak model just runs. The analogy even predicts the failure mode: habits are fast but brittle and misfire when the environment shifts, which is exactly how a macro rots when its surface drifts — and the fix is the same one biology uses, re-deliberate (the bail-out detector kicks the system back into System 2).
